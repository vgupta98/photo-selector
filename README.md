...
- **Burst grouping** — frames shot in a rapid burst (same camera, within ~2 seconds) collapse into one grid tile with a count badge, so a burst is a single decision instead of a dozen near-identical thumbnails. Click it to open just those frames side by side — Compare for two, Survey for more — and keep the sharpest.
...
In **Survey**, `Tab` and the arrow keys move the highlighted tile, `F` / `1`…`9` file it into a category, and `Esc` returns to the grid.

In the **grid**, `Cmd`-click or `Shift`-click tiles (or `Cmd+A`) to multi-select, then `F` / `1`…`9` file the whole selection into a category, **Copy photos to folder…** copies just the selection, **Delete** (or `Cmd`+`Delete`) moves the selection to the macOS Trash after a confirm, `C` opens the selection side by side (Compare or Survey), and `Esc` clears it.
