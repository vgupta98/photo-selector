...
class PhotoFeatureExtractor(
    ...
    suspend fun featuresFor(photo: Photo): PhotoFeatures? {
        ...
        // file or model id changes. A *persistent* per-inference failure therefore re-pays decode+embed
        // every pass by design (no negative caching) — acceptable because a model that can't infer at
        // all already falls back to the classical embedder at load time, so this path is rare.
