 ...
 *
 * Two distinct stored entries that share the same `(size, mtime)` and whose exact paths are
 * both gone resolve to the same single candidate and collapse into one favourite (the
...
object FavouritesResolver {
    fun resolve(entries: List<PhotoEntryDto>, scanned: List<Photo>): Set<PhotoId> {
        ...
        // Built lazily: in the steady state (post-v2 file, no rename) every entry resolves
